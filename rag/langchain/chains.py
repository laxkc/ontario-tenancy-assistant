...
def get_qa_chain(
    ...
    Args:
        ...
        temperature: Temperature for generation (default: 0.0 for factual)
    ...
    """
    print(f"🔧 Setting up QA chain with {model_name}")

    # Initialize OpenAI LLM
    llm = ChatOpenAI(
        model=model_name,
        temperature=temperature,
        api_key=config.OPENAI_API_KEY,  # Will be loaded from .env
    )
...
def get_simple_retrieval_chain(k: int = 5, filter: dict = None):
    """Create a simple retrieval chain (no LLM generation).
    ...
    Returns:
        Retriever chain that returns formatted documents
    """
    retriever = get_retriever(k=k, filter=filter)

    # Chain that just retrieves and formats
