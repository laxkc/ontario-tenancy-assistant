        ...
        for section in part.get("sections", []):
            ...
            if subsections:
                ...
                for subsection in subsections:
                    ...
                    if not subsection_text or len(subsection_text) < config.MIN_CHUNK_LENGTH:
                        continue

                    chunk_id = generate_chunk_id(
                        config.JURISDICTION_ABBR,
                        config.ACT_ABBR,
                        part_number,
                        section_number,
                        subsection_number
                ...
                if not section_text or len(section_text) < config.MIN_CHUNK_LENGTH:
                    continue
...
def main():
    ...
    if not config.INPUT_JSON.exists():
        print(f"\n❌ Error: Input file not found: {config.INPUT_JSON}")
        print(f"\nRun the parser first:")
        print(f"  cd ../data && python parse_document.py")
        return 1

    print(f"\n📄 Input:  {config.INPUT_JSON}")
    print(f"📁 Output: {config.RAG_READY_JSON}")
    print()
    ...
    oversized = [c for c in chunks if c["metadata"]["tokens"] > config.MAX_TOKENS_PER_CHUNK]
    if oversized:
        print(f"   ⚠️  Chunks exceeding {config.MAX_TOKENS_PER_CHUNK} tokens: {len(oversized)}")
    ...
    rag_data = {
        "metadata": {
            "source_file": str(config.INPUT_JSON),
            "act_name": document.get("act_name"),
            "jurisdiction": document.get("jurisdiction"),
            "citation": document.get("citation"),
            "total_chunks": len(chunks),
            "chunk_strategy": config.CHUNK_LEVEL,
            "embedding_model": config.EMBEDDING_PROVIDER
        },
        "chunks": chunks
    }

    with open(config.RAG_READY_JSON, "w", encoding="utf-8") as f:
        json.dump(rag_data, f, indent=2, ensure_ascii=False)

    print(f"✓ RAG-ready file created: {config.RAG_READY_JSON}")
    print(f"  Size: {config.RAG_READY_JSON.stat().st_size / 1024:.1f} KB")
